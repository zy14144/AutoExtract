...
# =========================================================
# 场景 9.7：加密分卷 + 卷1 伪装 .pdf + 前缀不一致（公式泳.pdf 场景）
# =========================================================
Write-Host "`n=== 场景 9.7: 加密分卷+伪装.pdf+前缀不一致 ===" -ForegroundColor Cyan
$s97 = Join-Path $testRoot 's97'; New-Item -ItemType Directory -Path $s97 -Force | Out-Null
New-RandomFile -Path (Join-Path $s97 'big.mp4') -SizeMB 6
& 'E:\Software\7-Zip\7z.exe' a -v4m -t7z -p@u@52 (Join-Path $s97 'p') (Join-Path $s97 'big.mp4') | Out-Null
Get-ChildItem $s97 -Filter 'p.7z.*' | Where-Object { $_.Name -match '\.0\d{2}$' -and $_.Name -match '\.001$' } | ForEach-Object {
    Rename-Item -LiteralPath $_.FullName -NewName 'p.001.pdf'
}
$out97 = Invoke-AutoExtract -Target $s97 -Password '@u@52'
Write-Result '加密伪装分卷解出 big.mp4 (6MB)' (Test-FileExists (Join-Path $s97 'big.mp4') 6291456)
# 目录批量模式下分卷文件是输入文件，设计上保留（绝不删除原始输入）；验证工作产物正确 + 无残留
Write-Result '卷1源文件保留（输入保护）' (@(Get-ChildItem $s97 -Filter 'p.001.pdf').Count -eq 1)
Write-Result '卷2源文件保留（输入保护）' (@(Get-ChildItem $s97 -Filter '*p.7z.002*').Count -eq 1)
Write-Result '无 work 残留' (Test-NoWorkDirs $s97)
